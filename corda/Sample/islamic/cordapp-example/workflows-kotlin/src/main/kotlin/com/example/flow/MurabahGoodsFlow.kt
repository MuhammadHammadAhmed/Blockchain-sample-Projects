...
class MurabahaGoodsFlow( val goodsId:String) :FlowLogic<SignedTransaction>() {
 ...
 */
        ...
        val inputGoods = goodsList.find { stateAndRef -> stateAndRef.state.data.internalReference == goodsId }
                ?: throw IllegalArgumentException("No proforma with ID $goodsId found.")
        val outputGoods= inputGoods.state.data.copy(assetOwner= borrower,owner=borrower,participants=listOf(borrower))

       // val outputGoods =inputGoods.state.data.copy(assetOwner=borrower,owner=borrower,participants=listOf(borrower))
        val moveCommand = Command(GoodsContract.Commands.Move(), listOf(ourIdentity.owningKey,borrower.owningKey))// TODO, try adding client  key

        val murabaha = MurabahaState(bank,borrower,agreementReference,date,outputGoods,costPrice,sellingPrice,annualProfitRate,tenor,true,true)
    ...
    //    progressTracker.currentStep = GENERATING_TRANSACTION
        //TODO add inputState
        builder.addInputState(inputGoods)
     ...
     //   return notarizedTx
        val stx = subFlow(CollectSignaturesFlow(ptx,setOf(borrowerSession),GATHERING_SIGS.childProgressTracker()))
        return subFlow(FinalityFlow(stx, setOf(borrowerSession), FINALISING_TRANSACTION.childProgressTracker()))
...
@InitiatedBy(MurabahaGoodsFlow::class)
class MurabahaGoodsResponder(val counterpartySession: FlowSession) : FlowLogic<Unit>() {
    @Suspendable
    override fun call() {
